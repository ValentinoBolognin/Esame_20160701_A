...
public class Model {
		...
		for(Driver d1 : this.graph.vertexSet()) {
			for(Driver d2 : this.graph.vertexSet()) {
				if(!d1.equals(d2)) {
					...
					if(vittorie>0) {
						Graphs.addEdge(this.graph, d1, d2, vittorie) ;
					}
				}
			}
		}
	 ...
	 * Il caso terminale è quando {@code passo==K}, ed in quel caso va calcolato il
	 * tasso di sconfitta.
	...
	private void ricorsiva(int passo, Set<Driver> team, int K) {
		...
		if(passo==K) {
			...
			// calcolare il tasso di sconfitta del team
